...
def combine_indices_pixelwise(stack, descs, indices, weights=None,
    ...
    """
    Combine multiple indices/bands into ONE pixel-wise time series:
      1. Extract each requested index's own (T_idx,H,W) sub-stack + its own
         DOY grid - different indices can be on different grids (e.g. S2's
         composite interval vs S1's).
      2. Interpolate every index onto a SHARED grid (the union of all
         individual grids) using the same linear weight-matrix approach as
         the main smoothing step.
      3. Normalize each index per-pixel (min-max over time) so indices on
         very different native scales (EVI ~0-1 vs a dB-scale SAR band,
         often negative) contribute comparably to the combined shape
         rather than the combination being dominated by whichever index
         happens to have the largest raw numbers.
      4. Average across indices (optionally weighted, e.g. by RF importance
         scores from Variable_Importance_RF) - NaN-aware, so a pixel where
         one index failed but others didn't still gets a sensible result.

    Processes pixels in chunks of `chunk_size` (same convention as
    vectorized_regrid_and_smooth) instead of ever building an
    (n_indices, T, H, W) array for the whole AOI at once - at province/
    country scale that 4D array is tens of GB even in float32, which is
    what was crashing before (3 indices × 27 timesteps × a 6645×6942 AOI
    needed ~28 GiB for *one* of several such temporaries). Peak memory now
    scales with chunk_size, not H*W; the only full-size array kept around
    throughout is the (T_common,H,W) output, the same footprint a
    single-index run already has.
    ...
    # Each index's own (T_idx, N) flat view + its interpolation weight
    # matrix onto the shared grid. The matrix is only (Tc x T_idx) - tiny,
    # independent of H/W - so it's built once here, not once per chunk.
    flats, weight_mats = {}, {}
    for idx in indices:
        doys = doy_grids[idx]
        flats[idx] = sub_stacks[idx].reshape(len(doys), N)
    ...
    for start in range(0, N, chunk_size):
        ...
        for idx in indices:
            chunk = flats[idx][:, start:end]
            Wm = weight_mats[idx]
            # Wm @ chunk always allocates a fresh array; the no-interpolation
            # branch needs an explicit copy too so the in-place ops below
            # never touch the original sub_stacks/full_stack data.
            aligned = chunk.astype(np.float32, copy=True) if Wm is None else (Wm @ chunk)
            ...
            valid = ~np.isnan(aligned)
            np.subtract(aligned, pmin, out=aligned)
            np.divide(aligned, rng_safe, out=aligned)   # aligned is now "normed", in place
