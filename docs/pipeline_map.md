...
| Seasonal hindcast/forecast (SEAS5) | ECMWF via CDS | Earthkit-based scripts in `agwise-planting-date-and-cultivar` (`AgWise_download.py` etc.) | One-time hindcast download + standardization not yet centralized | ✅ `get_seasonal()` (v0.3): per-year cached `Seasonal_<VAR>_i<MM>_<year>.nc`, valid-date time axis, AGRO units matching `get_climate` observations for bias correction (pending live CDS smoke test) |
