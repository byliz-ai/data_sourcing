...
| Soil & topography at trial points | EthioSIS / SoilGrids / SRTM rasters | `terra::extract` on locally stored projected rasters | Rasters assembled ad hoc per country | ✅ `extract_static_points()` (SoilGrids 2.0 + Copernicus DEM, nearest-valid fill) |
| Crop-model weather+soil files (DSSAT `.WTH`/`.SOL`, APSIM `.met`/soil) | Built by `readGeo_CM_zone.R` / `01_readGeo_CM_zone_APSIM.R`, copied per use case (fertilizer 113×, potentialyield ~200×) | Per-use-case R scaffolding: pivots, pedotransfer, texture triangle, DSSAT/apsimx writers all re-coded | Hundreds of near-identical copies; layer-index bugs; slow | ✅ `to_dssat()`/`to_apsim()` (v0.7): season-sliced weather + SoilGrids + Saxton-Rawls hydraulics → engine files, round-tripped through the DSSAT/apsimx readers |
| Growing-season slice (climate/NDVI to a planting→harvest window) | Whole years fetched then sliced by hand per module (cross-year seams hand-coded) | Fragile date arithmetic; repeated per use case | ✅ `get_season()` (v0.6): cross-year aware slice, region cube or per-trial points |
