...
| Seasonal hindcast/forecast (SEAS5) | ECMWF via CDS | Earthkit-based scripts in `agwise-planting-date-and-cultivar` (`AgWise_download.py` etc.) | One-time hindcast download + standardization not yet centralized | 🔜 roadmap (hindcast driver implementing Jemal's proposal: `Daily_<VAR>_<y0>_<y1>.nc` merged products) |
| Crop mask | ESA WorldCover via GEE | GEE crop-mask class in the legacy toolkit | Tied to GEE flow | 🔜 with the GEE driver |
