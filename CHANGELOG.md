...
(`resltn <- 0.05`) in both count (~23% at −13° lat) and locations — and
there was no way to reproduce the old behaviour through the API. New
`res_deg=` on `make_grid` (CLI `--res-deg`, R `ad_make_grid(res_deg=)`):
a fixed degree step on both axes, centres snapped to the global raster's
half-cell offsets (`.025/.075` for 0.05°), no latitude correction —
lines up with outputs generated by the original R pipeline. `res_km`
stays the default/preferred mode for new work; documented side-by-side
in REFERENCE §make_grid. +3 tests.
