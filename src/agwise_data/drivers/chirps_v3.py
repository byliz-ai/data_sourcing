...
The complete CHIRPS v3.0 daily series (1981-2025, 0.05 degree, ~24 GB/year;
the "rnl" ERA5-disaggregated branch) is staged in the AgWise
``Global_GeoData/Landing`` tree, so this source is
...
class ChirpsV3Driver(Driver):
    def _fetch_year(self, variable: str, year: int, domain: str):
        ...
        raise RuntimeError(
            ...
            "(Rainfall/chirps_v3/<year>.nc; see the chirps_v3 catalog entry "
            "for the staged year range). Set AGWISE_LOCAL_ROOT to the AgWise "
            f"Global_GeoData/Landing folder — no local file found for {year}. "
            "For downloadable rainfall use the default CHIRPS v2.0 source."
        )
